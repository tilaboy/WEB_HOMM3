...
`core/` 完全不依赖 DOM，所以整套战斗规则、地图生成和 AI 回合都能在 Node 里直接跑测试 —— 这是 334 项断言和数值平衡实验的基础（AI 的 120 天长局回归就是这么跑的）。
...
M1 探索 · M2 城建 · M3 六边形战斗 · M4 魔法 · **M5 开局设置 + 电脑对手** 已完成（存档 v6）。设计决策与数值实测记录在 `homm-web/DESIGN.md`。
