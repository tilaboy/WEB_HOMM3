 ...
 * 右侧查看器（HOMM3 布局）。
 *
 * ★ `#156`（D-1「面板内分段」，`in-game-ia.md §3.3` + `roadmap.md` 口径索引）—— 面板 = **常驻核 + 分段**：
 *   · **常驻核** = `head`（肖像 / 名 / 等级+经验 / **移动力+法力** / `魔法书` 图标 / `宝物` 开关挂在肖像上） + **tab 条**；
 *   · **段 = {部队 / 属性 / 城池 / 势力}**（**tab 条**、**默认 = 部队**，任何场景不例外）；
 *     `hp-teach` 落「属性」段首行（**不算常驻核**）；`hp-towns` 段内一行摘要；`hp-factions` 不删、进段。
 *   · tab 控件**复用既有 `segment` 控件语言**（`.ss-seg` / `.ss-seg-item`），**不新造第二套**。
 *   · ⚠️ **已作废**（旧形，留痕）：~~常驻核 = `head`+`bars`+`army` ≈ 239~~、~~分段 = 属性/城池/势力、默认 = 属性~~。
 *     **取代者 = `roadmap.md` 口径索引「`#156` 英雄栏削减（第五次也是最后一次更正）」一行。**
 *   · **判据**：对象 = **`#side.scrollHeight ≤ #side.clientHeight`**（**不是 `sideBody`** —— 它 `overflow:visible`
 *     ⇒ `clientHeight ≡ scrollHeight` ⇒ **恒真 = 墙**），**默认段 + 每 tab 各量一次**；
 *     硬① tab 条不被滚出视野；硬② 默认段 `#side` 不滚；非默认段只许**段内**滚。量尺 `tools/hpbudget.mjs`。
...
export class HeroPanel {
  ...
  private headSection(state: GameState, hero: Hero): HTMLElement {
     ...
     * ⚠️ **以下这句已作废（留痕，勿据此实现）**：
     *   ~~处置 =「`宝物` 移到**「部队」段首行**」~~ —— 最终实现是把 `宝物` 挂到**肖像**（见 `headSection`
     *   的 `hp-face`；`armySection` 里 `:258-265` 明说"不在这里"）。理由：放段首行会给**默认段**加 ~44px，
     *   与硬②（默认段 `#side` 不滚）相抵；挂肖像则零宽度、零高度、仍是 ≥40×40 真实目标。 */
