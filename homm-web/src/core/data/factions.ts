...
export const DIFFICULTIES: Record<Difficulty, DifficultyDef> = {
  ...
  hard: {
    id: 'hard',
    name: '困难',
    // 注意：playerGrowthBonus 已拍板为 0（决策②，与 normal 相同），所以这里绝不能写"周增长受压"——
    // 否则给玩家看的是不存在的惩罚。强度由开局 ×0.7 / 移动力 ×0.9 / 野怪 ×1.25 / AI 更早出击承担。
    desc: '你起始资源紧张、移动力受压、野怪更厚；电脑对手资源更多、增长更快，且更早发动进攻。',
    startMul: 1.25,
    growthBonus: 0.2,
    aggression: 0.8,
    playerStartMul: 0.7,
    // 决策②：不给负向周增长（用户拍板）——0 表示与普通档相同（加法系数为 0，不罚）。
    // 避免天使周产 2→1 腰斩，更重要的是不让进行中的旧存档中途被加难度。
    // 强度由开局 ×0.7 / 移动力 ×0.9 / 野怪 ×1.25 / AI 更早出击承担。
    playerGrowthBonus: 0,
    playerMoveMul: 0.9,
    monsterMul: 1.25,
  },
