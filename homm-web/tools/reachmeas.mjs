 ...
 *
 * ## ★★ 本工具的两"列"不是一回事：**逐类列可信、全图行不可** —— 差别是「**墨核对动画免疫**」
 *   · **逐类列（"按地形分档"）= 动画免疫**：它的谓词是 `after ≈ 众数变化色 ±3`（**窄颜色窗**）
 *     ⇒ 水波 / 选中脉冲那些"**小 Δ 的动画像素**"**进不了这个窗** ⇒ 水面怎么漂，这一列的 `n` 与中位**都不动**。
 *     （实测旁证：冻帧对上"水"在该列只 `n=5378`、变化仅占水面积 `2.7%`；而"不冻"时水面上变化 ≈ 整片水面。）
 *   · **全图行（"全部变化像素"）= 不免疫**：任何**会动**的像素都算变化 ⇒ 换一帧就多出成千上万个
 *     ⇒ 该行是**帧的函数**，不是染色层的函数 ⇒ **引用它必须写明"已冻帧"**（冻结 = 打平 `performance.now()`）。
 *   ⇒ 用法纪律：**引用逐类列，要连"它是 `±3` 墨核"一起引；引用全图行，要连"这是冻帧"一起引。**
 *     两者都**不是**"该类有多少像素" —— 把逐类列的 `n` 读成"该类的像素数"会把"暗类只有 15 个像素"这种**假象**传出去（见下块）。
 *
 * ## ★ 逐类列的**正名** = 「`(α)` 基线定义 · 墨单带」—— **它结构上看不到 `(ii)` 的亮芯**
 *   它的谓词 `isInk()` 只认**暗像素** ⇒ 亮极（`NOGO_EDGE_GLOW = '#fff3c8'`）**永远落不进这一列**。
 *   ⇒ **判 `(ii)`（双色 halo）请用 `tintab` 的「逐类 × 逐带」四带中位（契约第 6 条）**；
 *     **本列只作 `(α)` 基线的可比列**（hence 输出里那行标题已写明 "【(α) 基线定义 · 墨单带 —— 判 (ii) 请用 tintab 的逐类×逐带】"）。
 *   ⇒ ⛔ **不要**把它改造成"按最近色归族"——那会造出**第二套族定义**（本会话已为"两套定义两个数"吃过一次）。
 *     要看"这对到底在量哪一族"，用本文件的 **`REACH_FAMILIES=1` 诊断**（默认输出一字不改，且会打印族占比最大者）。
 *   ⇒ ⚠️ **该列的"众数变化色"是 A/B 的函数**：换成 `?devtint=0`（染色开 vs 关）那种对，
 *     众数变化色会变成**蓝水洗**（实测：沙/土 该列 `n=26986`、中位 `1.02`）⇒ **更证明它只能钉在 `(α)` 那一对上**。
 *
 * ## ⚠️ 已核：本表「按地形分档」那一列的人口 = **`±3` 墨核**（地形偏置，勿当全部）
 *   本文件**逐类那一列**只统计 `after ≈ 众数变化色(±3)` 的像素（= 污染检查留下的"纯墨核"）。
 *   实测（`i_reach_*` 身份帧，2026-09-21；⚠️ **该对是「跨构建对」**：`before` = `68abf37`、`after` = `3e8084f`
 *     ⇒ 下列**只用于说明 `±3` 谓词的"人口结构"**（谓词对暗底的偏置方向），**不得当作同构建 A/B 的量级**）：该 `±3` 门槛**对暗底偏置** —— 暗底上墨达不到纯墨色
 *   （`after` 落在比"墨"和"底"都更暗的一簇，如 `rgb(24,18,17)`），于是**暗类的墨核被大量漏掉**：
 *   暗类实际变化 `11094(午)/10785(夜)`，其中 `±3` 墨核只有 `15(午)/517(夜)`。
 *   ⚠️ **引用这条时必带保留**：那对图是**跨构建** A/B（`68abf37` ↔ `3e8084f`）⇒ 它的**数量不成立**
 *     （另有独立作废理由：`before` 用"没选择英雄"⇒ population 被水洗灌满）。**此处只用它说明「±3 谓词
 *     的人口结构」**（谓词锚在标称色上 ⇒ 暗底上真正的墨线被漏掉），**不得当同构建 A/B 引用**。
 ...
 *
 * ## ★ 本列的正名（team-lead 裁 · 契约第 6 条）：**「`(α)` 基线定义 · 墨单带」**
 *   · 本文件**只用「墨」这一条带** ⇒ 它是 **`(α)` 时代的可比列**（`(α)` 只有红+墨）。
 *   · ⛔ **它结构上量不到 `(ii)` 的亮芯**（亮芯像素永远进不了 `±3` 墨核）⇒ **不得用它判 `(ii)`**。
 *   · **④ 的「逐类 × 逐带」验收数（中位 WCAG）→ `tools/tintab.mjs`**（draw-time 族标记，`fill/edge/glow/ink`）；
 *     **本文件 = 全图 A/B 旁证腿 + `(α)` 墨单带基线列**（两者都保留、留痕不删）。
 *   · 独立腿用词纪律：同工具换视口/通道 = **「通道独立」**，**不得写成「独立复核」**。
 ...
 *
 * ## ★★ 为什么「逐类那一列」可以信、「全图变化像素那一行」不可以（**墨核对动画免疫**）
 *   这是本工具**最容易被误用**的地方，team-lead 明令写进头注：
 *   · 「**全部变化像素**」那一行 = **跨两张 PNG 差分** ⇒ 任何**帧间会动**的东西（水面 4 帧循环、
 *     选中脉冲 `performance.now()` 正弦）都落进 population ⇒ **该行暴露于动画污染**；
 *     `SHOT_FREEZE=1` 未开时**不得引用该行**（`5ed22d3` 的实测：`edge` 模式 51125 个变化像素里只有 2 个是红）。
 *   · 而**逐类那一列**的人口谓词 = `after ≈ 众数变化色 ± 3`（= **墨核**）⇒ 动画像素（水波/脉冲）的 `after`
 *     **不落在墨核上** ⇒ **进不了这一列** ⇒ **该列结构上对动画免疫**。
 *     实测（`i_reach_*`，冻帧）：水面积 `283 098`，其中变化像素只 `7 537`/`7 535` = **该类的 2.7%**。
 *   · ⇒ **一句话：这个工具的「逐类列」可以信，「全图行」必须先冻帧。**
 *     （`engineering-lead-2` 用**另一种方法**独立复算过同结论 —— 那是**可复现性**，不是独立性。）
 *   · ⚠️ **但「逐类列」的另一半代价**：它是**墨单带**谓词 ⇒ **结构上看不到 (ii) 的亮芯**（亮极不是暗像素）。
 *     契约第 6 条 ⇒ 该列**只作 `(α)` 基线可比列**，**不作 (ii) 判据**（(ii) 判据列在 `tintab`）。
 *     真要看 (ii) 的四个族，用 `REACH_FAMILIES=1`（见文件末尾诊断块）。
 *
 * ## ★ 优先的同一变量 A/B 来源 = `?devtint=0`（`6dd5631`）
 *   不要再拿两批不同来源的截图拼 A/B（`i_reach_*` 那对就是**跨构建** `68abf37`↔`3e8084f`）。
 *   正确做法：**同一构建、同一次会话、同一帧、`SHOT_FREEZE=1`**，只差 `&devtint=0`
 *   ⇒ population = 整层 ④（蓝填 + 红 + 亮芯 + 暗墨）⇒ 族诊断四族齐全、守恒可查。
 *   ⚠️ 此时**逐类那一列会失效**（谓词锚在"众数变化色"= 蓝填上，读到的是水洗，不是墨）—— 别引它。
 *
 * 用法：node tools/reachmeas.mjs <after.png> <before.png> <label> [身份帧底图.png]
 *   （第 4 参 = **分类用的身份帧**（正午 `none` 底图）；缺 ⇒ 退回同相位并告警。
 *     `REACH_FAMILIES=1` 另打「族 × 地形」诊断，**默认输出一字不改**。）
 */
...
console.log(`  ── 墨像素按"其底地形"分档【(α) 基线定义 · 墨单带 —— 判 (ii) 请用 tintab 的逐类×逐带，见文件头】`);
