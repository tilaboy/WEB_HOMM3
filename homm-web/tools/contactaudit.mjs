 ...
 * ★ **两个判据、用途不同、不可互替**（team-lead 2026-09-21 要求分开写清）：
 *   · **`M` = 0（且**非台账**）** = **「能开窗」**的判据 —— 要的是一个**静默状态**；
 *   · **跑内 delta = 0**          = **「这一次读数可信」**的判据 —— 只算**跑内增量**。
 *   ⇒ **两者都必要、不互相替代**。（"被改过" ≠ "在跑内被改"。）
 *
...
if (TREE0 === null || TREE1 === null) {
  /* 取不到工作树 ⇒ **说清楚"没核"**，别让人误以为核过（静默 = 假信心）。 */
  WARNL('旁证未能核验：取不到 `git status --porcelain`（非 git 仓库 / 无 git）—— 本次读数**未**做「跑内工作树增量」核验（主判据=构建指纹**已**核）。');
} else if (TREE0 !== TREE1) {
