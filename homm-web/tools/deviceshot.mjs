 ...
 *      ⚠️ **诚实边界（2026-09-21 自测）**：本通道**未能复现**它 —— `SHOT_NOWARM=1` vs 默认 = **`0/0` 逐字节同**。
 *      最可能的原因 = 本通道常配的 `SHOT_FREEZE=1` **把每一帧冻成同一张**（冻帧下"抓早/抓晚"本就无差）
 *      ⇒ **该守卫在冻帧场景下无法被本通道检验**。它是**继承 `-2` 的 CDP 发现做的廉价保险**（成本＝一次空抓），
 *      **不是**我在这里测到的效果 —— **不夸大战功**。
 *
 ...
 *      ⚠️ **实测更正（2026-09-21 · `art-director`）：不是"大概率躲过"，而是"基本躲不过"** ——
 *      本工具等 `frames(45)` ≈ 0.75s 就抓，**此刻 toast 通常还在** ⇒ **默认截图里就带着这块 toast**：
 *      「隐 vs 不隐」= `|Δ|>4` **38 268 px**（**99.9% 落在 toast 框内**）+ `Δ≠0` **362 308 px**。
 *      **好在**默认配置**逐字节确定**（同配置复跑 `0/0`）⇒ toast 是**稳定元素**、A/B 两侧**相消**
 *      ⇒ **只要两次抓帧配置相同就无害**；**危险的是"一侧隐、一侧不隐"** —— 会凭空差出那一块。
 *      （`-2` 侧它曾把 792 `dark` 的 `(a)并集` 抬高 `31059`。）
 *      **修法**：新文档注入 `#hint{display:none !important}` —— ⚠️ **但实测它"不是逐像素局部"**：
 *      除 toast 框内那 38 268 px 外，**全帧还有 ≤4 LSB 涟漪**（`Δ≠0` 362 308 px、bbox 铺满 2376×960、
 *      `max|Δ|=177` 只出现在框内；**涟漪的机制未定，不写成"整幅地图位移 1px"这种未验结论**）。
 *      安慰剂对照（inert 规则）= **`0/0`** ⇒ 涟漪**不是**"多了一次注入"造成，而是**隐藏 toast 本身**引起。
...
try {
  ...
  // ④ 抓帧免疫①：把 `#hint` 启动 toast 从**渲染结果**里摘掉（DOM 覆盖层，**默认关**；`SHOT_HIDEHINT=1` 才隐藏）。
  //    注入时机 = 新文档、`document` 一有根节点就插 `<style>` ⇒ 早于 `main.js` 跑，
  //    因此 toast 元素**即使后来才被创建**也天然不带显示。
  //    ⚠️ 原注写的"**只影响那一层，不改布局**"**已被实测推翻**（见下）。
  //  ⚠️⚠️ 2026-09-21 **实测更正：隐藏 `#hint` 不是"逐像素局部"改动 ⇒ 本守卫改为「默认关」**。
  //     本通道实测（独立进程 · `SHOT_FREEZE=1` · 同 URL · 同构建 `dist/render/MapRenderer.js` `76415ae9`；
  //     用 `node tools/diffbox.mjs` **两种阈值都报** —— 报数必须连阈值）：
  //       · `|Δ|>4` = **38 268 px**，bbox `x[982..2351] y[699..956]`、涉及 **99/960 行**，**99.9% 落在 toast 框内**；
  //       · `Δ≠0`  = **362 308 px**，bbox **铺满整帧** 2376×960 ⇒ 那 32 万**是 ≤4 LSB 的全帧涟漪**，不是内容改变；
  //       · 画布几何完全相同（两跑都打印 `canvas 2376x720 @css 792x240 dpr 3`）。
  //     ⇒ 结论：**实质改动 = toast 那一块** + **一层全帧 ≤4 LSB 涟漪（机制未定）**。
  //     **同批对照组**：`SHOT_NOWARM=1` vs 默认 = `0/0`；inert 规则 `#__nope_never_exists{display:none}` vs "无注入" = `0/0`；
  //     默认 vs 默认复跑 = `0/0`（都逐字节同）⇒ 该涟漪**不是**"多了一次注入 / 时序变了"造成。
  //     ⇒ 故：`SHOT_HIDEHINT=1` 才隐藏；**默认保留**（一个**连字节都改**的改动不能当静默默认值 ——
  //        否则凡与历史读数比较，都是在比"两个不同场景"）。要隐藏时，**A/B 两侧必须同样隐藏**（对称）。
  //     ⚠️ **反向推论（同样是结论）**：默认抓帧里 **toast 是"在的"**（不是"赛跑躲过"）⇒ 同配置两次抓帧都带它、
  //        差分时**相消** ⇒ 配置一致即无害；但**别**把"默认 vs 隐"当成"只差被测变量"的干净 A/B。
