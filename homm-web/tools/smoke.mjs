...
// 2. 深夜显著压暗且偏蓝，但不能黑到影响读图（下限 70% 亮度，§2.3 L1）
//    判据按「2×容差 < 可区分间距」设，同时守住三件事：比正午暗 / 偏蓝 / 不黑到读不清。
{
  const t = lightTintAt(0.68);
  // r ≤ 200：正午 r=255 ⇒ 会红（非恒真）；新值 178 ⇒ 余量 22。
  // b − r ≥ 12：新值 Δ=27 ⇒ 余量 15；若有人把 tint 调成 (178,182,178) ⇒ Δ=0 ⇒ 会红。
  ok(t.r <= 200 && t.b - t.r >= 12, `深夜压暗且偏蓝（rgb(${t.r},${t.g},${t.b})）`);
  // r ≥ 165：新值 178 ⇒ 余量 13；若回退到旧 0.58（148）⇒ 会红。
  ok(t.r >= 165, `深夜不黑过下限（r=${t.r} ≥ 165，§2.3 L1 floor 0.70）`);
}
