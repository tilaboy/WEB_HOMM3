...
**它是什么**：`terrainShade.ts` 新增「地貌区场」（≈14 格/团，比地貌层的 ≈6 格/团**更大尺度**）⇒ 把地图切成 **4 个「地貌区」**，每区一套**色偏**（明暗 + 冷暖）+ 区域化砖变体选择 ⇒ 目标是从"一整张均匀草地"变成"**一眼认出这是哪片地**"。开关 = `?devregion=0` 关 / 缺省开。**同一份 dist 批抓**（`dist/main.js` md5 抓前抓后一致）。
