...
> - **已实现并提交（触屏体验）**：第 6 节 **M-08 横屏 / M-09 触控热区 / M-11 双击居中**，以及 **M-10 的 web 侧**（`a668e63`；M-09 分级残余 `17e1b29`）。
> - **⚠️ M-07 安全区：CSS 已提交，但 2026-09-18 真机实测「未生效」** —— Android 16（API 36）上 WebView 拿到零 insets，画面出现白带（顶 74px / 底 63px / 左 136px）。**不要再把 M-07 当作已完成项**，详见第 6 节 M-07 行与 §8.1。
...
| **M-07** | P1 | 安全区：`public/index.html` 加 `viewport-fit=cover`；`style.css` `#app` 加四边 `env()` padding；顶/底栏高度改 `calc(...)` | 刘海/灵动岛/挖孔/手势条遮挡 HUD | 无；`audit:layout` 是**纯生成器审计，不产截图、不做图像比对**（已实测 640 局全过）——**不存在"截图基线"**（原稿此处判断有误，已更正） | ◐ **CSS 已实现 —— 但 2026-09-18 真机实测「未生效」**（`a668e63`）——`index.html:7` 加 `viewport-fit=cover`；`style.css` 三处改 **`var(--safe-area-inset-*, env(...))`**（`#app:63-66` / `#rotate-hint:775` / `#start-screen:1381-1384`，理由见 §5.1 提示）。**🔴 真机证据（Android 16 / API 36 模拟器，AVD `h3land`）**：`SystemBars` 插件确已注册、**App 窗口本身是 full-bleed**（`frame=[0,0][2400,1080]`），但截图实测存在 **WebView 默认白底露出的白带**：**顶 74px（28.2dp）· 底 63px（24.0dp）· 左 136px（51.8dp）**；同批次日志显示**内容区被内缩 `(136,74)` 而 insets 上报为 `Rect(0,0,0,0)`** → `var()` 与 `env()` 都拿到 **0**，CSS 侧无处补偿。**根因尚未坐实**（三种可能：WebView 布局边界、insets 下发、窗口背景色），但可确定**当前写法在安卓上等于没生效**。完整证据：`.workbuddy/artifacts/emu-evidence-android36.txt` |
...
| 项 | 说明 |
|---|---|
| **M-07 安卓安全区注入路径** | ~~须真机验证~~ → **2026-09-18 已在 Android 16 模拟器上验证：未生效。** 代码用 `var(--safe-area-inset-*, env(...))` 写法正确，但真机上 WebView 拿到的是**零 insets** —— 实测白带 顶 74px / 底 63px / 左 136px，状态栏时钟呈**浅灰画在白底上**（几乎不可见），底部是白色导航栏 + 手势条。窗口本身 full-bleed 已确认成立，故问题不在 CSS 写法。**待修（见 §8.1 同条与 roadmap G-12）** |
